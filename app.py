...
fig2.add_annotation(
    text=f"Paid Apps<br>{paid_percent:.2f}%",
    x=0.35, y=1.17,
    font=dict(size=18, color="white"),
    showarrow=False
)
...
revenue_by_category = apps_df.groupby('Category')['Revenue'].sum().nlargest(6)
...
def styled_heading(text):
    st.markdown(
        f"""
        <h1 class="centered_heading" style="font-size:25px; color:white; text-align:center;">
            {text.upper()}
        </h1>
        """,
        unsafe_allow_html=True
...
with col7:
    ...
    styled_insight("Family and Lifestyle categories dominate revenue generation, with a sharp drop after Medical—indicating strong monetization concentration in a few categories.")
